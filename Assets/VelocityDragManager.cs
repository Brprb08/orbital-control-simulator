...
public class VelocityDragManager : MonoBehaviour
{
    ...
    private void StartDrag()
    {
        ...
        if (dragSphereObject == null)
        {
            ...
            // OPTIONAL: Do NOT parent it to the planet 
            // if you want to keep it separate from planet's localScale.
            // If you prefer it as a child, do SetParent with "false" 
            // so local transform is not changed by parent's scale.
            // dragSphereObject.transform.SetParent(planet.transform, false);

            dragSphereCollider = dragSphereObject.AddComponent<SphereCollider>();
            dragSphereCollider.isTrigger = true;

            // Put it on a unique layer if needed
    ...
    private void UpdateDrag()
    {
        ...
        // We'll do "far side" intersection for demonstration
        ...
        if (intersectionPoint != Vector3.zero)
        {
            ...
            // ★ NEW/CHANGED: We ONLY capture direction (ignore the "stretch" for speed)
            Vector3 rawDirection = intersectionPoint - sphereCenter;
