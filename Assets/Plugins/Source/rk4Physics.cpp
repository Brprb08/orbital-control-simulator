    ...
    Vector3* bodies, float* masses, int numBodies, int centralBodyIndex, Vector3 thrustImpulse)
{
    ...
    k1_v.x += thrustAcc.x; k1_v.y += thrustAcc.y; k1_v.z += thrustAcc.z;
    Vector3 k1_x = vel;
    ...
    k2_v.x += thrustAcc.x; k2_v.y += thrustAcc.y; k2_v.z += thrustAcc.z;
    Vector3 k2_x = vel_k1;
    ...
    k3_v.x += thrustAcc.x; k3_v.y += thrustAcc.y; k3_v.z += thrustAcc.z;
    Vector3 k3_x = vel_k2;

    Vector3 pos_k3 = { pos.x + dt * k3_x.x, pos.y + dt * k3_x.y, pos.z + dt * k3_x.z };
    Vector3 vel_k3 = { vel.x + dt * k3_v.x, vel.y + dt * k3_v.y, vel.z + dt * k3_v.z };
    Vector3 k4_v = ComputeAcceleration(pos_k3, mass, bodies, masses, numBodies, centralBodyIndex);
    k4_v.x += thrustAcc.x; k4_v.y += thrustAcc.y; k4_v.z += thrustAcc.z;
    Vector3 k4_x = vel_k3;

    vel.x += dt * (k1_v.x + 2.0f * k2_v.x + 2.0f * k3_v.x + k4_v.x) / 6.0f;
    vel.y += dt * (k1_v.y + 2.0f * k2_v.y + 2.0f * k3_v.y + k4_v.y) / 6.0f;
    vel.z += dt * (k1_v.z + 2.0f * k2_v.z + 2.0f * k3_v.z + k4_v.z) / 6.0f;

    pos.x += dt * (k1_x.x + 2.0f * k2_x.x + 2.0f * k3_x.x + k4_x.x) / 6.0f;
    pos.y += dt * (k1_x.y + 2.0f * k2_x.y + 2.0f * k3_x.y + k4_x.y) / 6.0f;
    pos.z += dt * (k1_x.z + 2.0f * k2_x.z + 2.0f * k3_x.z + k4_x.z) / 6.0f;
}

extern "C" __attribute__((visibility("default")))
void RungeKuttaSingle(Vector3* position, Vector3* velocity, float mass,
    Vector3* bodies, float* masses, int numBodies, float dt, Vector3 thrustImpulse)
{
    int centralBodyIndex = 0;
    if (mass <= 1e-6f) return;
